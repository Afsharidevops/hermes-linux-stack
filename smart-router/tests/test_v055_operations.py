...
def test_control_schema_upgrades_in_place(tmp_path):
    ...
    assert db.schema_version() == "0.5.5"
    # New v0.5.5 tables are created without changing the compatibility filename.
    tables = set(db.engine.dialect.get_table_names(db.engine.connect()))
    assert {"v55_runtime_settings", "v55_access_groups", "v55_skills", "v55_agent_skills"} <= tables
