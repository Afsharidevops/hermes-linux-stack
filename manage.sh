...
n8n_api_check() {
  ...
  chmod 600 "$env_file"
  printf 'N8N_API_KEY=%s\n' "$key" > "$env_file"
  if "${DOCKER[@]}" run --rm --network hermes-omniroute-net \
...
run_n8n_verifier() {
  ...
  if [[ ",$profiles," == *,smart-router,* ]]; then
    ...
    router_health_url="http://omniroute:20128"
  fi
  ensure_stack_secrets_dir || return 1
...
case "$command" in
  ...
  logs)
    case "${2:-}" in
      ...
      *) printf 'Choose hermes, omniroute, smart-router, webui, n8n, or caddy.\n' >&2; exit 2 ;;
    esac
    ;;
  doctor)
...
' <<< "$rendered_json")"
  ...
  set-upstream-terminal)
    ...
    if [[ "$state" == enabled ]]; then
      ...
      printf '  the Telegram bot token, omniroute key, API server key, and n8n Instance token.\n'
...
PY
  ...
  set-n8n-instance-mcp-token)
    require_profiles omniroute hermes n8n
    if [[ -n "${2:-}" ]]; then
  ...
  set-backend-api-key)
    key="${2:-}"
    if [[ -z "$key" ]]; then
      read -r -s -p 'OmniRoute upstream API key: ' key; printf '
' >&2
    fi
    [[ -n "$key" ]] || { printf 'API key is required.
' >&2; exit 2; }
    profiles="$(sed -n 's/^COMPOSE_PROFILES=//p' "$ENV_FILE")"
    replace_env_value "$ENV_FILE" SMART_ROUTER_UPSTREAM_API_KEY "$key"
    if [[ ",$profiles," == *,smart-router,* ]]; then
      compose up -d --no-deps --force-recreate smart-router
      printf 'OmniRoute upstream key updated for Smart Router. Hermes/Open WebUI keep SMART_ROUTER_CLIENT_API_KEY.
'
    else
      [[ -f "$HERMES_ENV" ]] && replace_env_value "$HERMES_ENV" OMNIROUTE_API_KEY "$key"
      replace_env_value "$ENV_FILE" OPENWEBUI_OPENAI_API_KEY "$key"
      compose up -d --no-deps --force-recreate hermes open-webui 2>/dev/null || true
      printf 'Direct OmniRoute client key updated.
'
    fi
    printf 'If OmniRoute API-key enforcement is desired, set OMNIROUTE_REQUIRE_API_KEY=true and recreate OmniRoute.
'
    ;;
