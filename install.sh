...
chmod 600 .env
mkdir -p data/omniroute data/hermes data/open-webui data/caddy data/smart-router data/n8n data/stack-secrets
...
secret_keys={
    'OMNIROUTE_INITIAL_PASSWORD','OMNIROUTE_JWT_SECRET','OMNIROUTE_API_KEY_SECRET',
    'OMNIROUTE_MACHINE_ID_SALT','SMART_ROUTER_HMAC_SECRET','OPENWEBUI_SECRET_KEY','N8N_ENCRYPTION_KEY'
}
secret_keys.update({"OMNIROUTE_STORAGE_ENCRYPTION_KEY","OMNIROUTE_WS_BRIDGE_SECRET"})
out=[]
for line in lines:
...
OMNIROUTE_API_KEY=local-internal
